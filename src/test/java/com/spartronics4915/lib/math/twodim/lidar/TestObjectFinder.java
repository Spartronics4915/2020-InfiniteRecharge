    ...
    public void interactivePointcloudTest()
    {
        ...
        var pointCloudCanvas = new Canvas()
        {
            ...
            public void paint(Graphics g)
            {
                ...
                try
                {
                    synchronized (mPointcloud)
                    {
                        ...
                        int circleDiameterCentimeters = (int) Math
                            .round(circleRadiusMeters * 100.0 * 2.0);
                        // var centers = finder.findSquares(pointcloud, new Translation2d(), 0.28,
                        // mNumVotesNeeded, 3, 0.3);
                        var centers = finder.findCircles(mPointcloud, circleRadiusMeters,
                            mNumVotesNeeded, 3);
                        ...
                        g.drawOval(toScreenCoordinates(center.getX() - circleRadiusMeters, true),
                            toScreenCoordinates(center.getY() - circleRadiusMeters, false),
                            circleDiameterCentimeters, circleDiameterCentimeters);
                        // break;
                        // }

                        g.drawString("Edge: " + mEdgeDetectorValue + ", Votes: " + mNumVotesNeeded,
                            10, 10);
                    }
                }
                catch (Exception e)
            ...
            private void drawPoint(Translation2d point, int quality, Graphics g)
            {
                ...
                g.drawOval(toScreenCoordinates(point.getX(), true),
                    toScreenCoordinates(point.getY(), false), 1, 1);
            }
        };
        ...
        }, new RobotStateMap(), new Pose2d(Units.inchesToMeters(-5.5), Units.inchesToMeters(-14),
