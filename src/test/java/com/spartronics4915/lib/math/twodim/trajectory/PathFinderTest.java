...
class PathFinderTest
{
    ...
    public void testPathFinder()
    {
        ...
        var pathFinder = new PathFinder(robotSize,
            new Rectangle2d(
                new Translation2d(inchesToMeters(140.0), inchesToMeters(85.25)),
                new Translation2d(inchesToMeters(196.0), inchesToMeters(238.75))),
            new Rectangle2d(
                new Translation2d(inchesToMeters(261.47), inchesToMeters(95.25)),
                new Translation2d(inchesToMeters(196.0), inchesToMeters(238.75))),
            new Rectangle2d(
                new Translation2d(inchesToMeters(196.0), inchesToMeters(85.25)),
                new Translation2d(inchesToMeters(211.0), inchesToMeters(238.75))));

        double start = System.currentTimeMillis();

        var startPose = new Pose2d(feetToMeters(1.54), feetToMeters(23.234167),
            Rotation2d.fromDegrees(0));
        var endPose = new Pose2d(feetToMeters(23.7), feetToMeters(27 - 20.2),
            Rotation2d.fromDegrees(0));
        ...
        var traj = TrajectoryGenerator.defaultTrajectoryGenerator.generateTrajectory(
            startPose, interiors, endPose,
            Arrays.asList(new CentripetalAccelerationConstraint(4.0)), 0, 0,
            feetToMeters(10), feetToMeters(4), false);
        ...
        var chart = new XYChartBuilder().width(1800).height(1520)
            .title(fm.toString() + " seconds.").xAxisTitle("X").yAxisTitle("Y").build();

        chart.getStyler().setMarkerSize(8);
        chart.getStyler()
            .setSeriesColors(new Color[]
            {Color.ORANGE, new Color(151, 60, 67)});
        ...
        chart.addSeries("Trajectory",
            refList.stream().map((it) -> it.state.state.getTranslation().getX())
                .collect(Collectors.toList()),
            refList.stream().map((it) -> it.state.state.getTranslation().getY())
                .collect(Collectors.toList()));
        ...
        chart.addSeries("Restricted Areas",
            restricted.stream()
                .flatMap((it) -> Arrays
                    .asList(it.getBottomLeft().getX(), it.getTopLeft().getX(),
                        it.getBottomRight().getX(), it.getTopRight().getX())
                    .stream())
                .collect(Collectors.toList()),
            restricted.stream()
                .flatMap((it) -> Arrays
                    .asList(it.getBottomLeft().getY(), it.getTopLeft().getY(),
                        it.getBottomRight().getY(), it.getTopRight().getY())
                    .stream())
                .collect(Collectors.toList()));
