...
public class TrajectoryIntegrationTest
{
    ...
    public void testPathFollower()
    {
        ...
        final DifferentialDrive kDifferentialDrive = new DifferentialDrive(kRobotLinearInertia,
            kRobotAngularInertia, kRobotAngularDrag, kDriveWheelRadius, kDriveTrackWidth,
            new DCMotorTransmission(kDriveWheelRadius, kRobotLinearInertia, kDriveLeftVIntercept,
                kDriveLeftKv, kDriveLeftKa),
            new DCMotorTransmission(kDriveWheelRadius, kRobotLinearInertia, kDriveRightVIntercept,
                kDriveRightKv, kDriveRightKa));
        ...
        var traj = TrajectoryGenerator.defaultTrajectoryGenerator.generateTrajectory(
            ...
            Arrays.asList(new DifferentialDriveDynamicsConstraint(kDifferentialDrive, 10.0)), 0.0,
            0.0, 3.65, 1.83, false);
    ...
    public void testSplineTrajectoryGenerator()
    {
        ...
        List<Pose2d> waypoints = Arrays.asList(new Pose2d(0.0, 0.0, Rotation2d.fromDegrees(0.0)),
            new Pose2d(Units.inchesToMeters(36.0), Units.inchesToMeters(0.0),
                Rotation2d.fromDegrees(0.0)),
            new Pose2d(Units.inchesToMeters(60.0), Units.inchesToMeters(100),
                Rotation2d.fromDegrees(0.0)),
            new Pose2d(Units.inchesToMeters(160.0), Units.inchesToMeters(100.0),
                Rotation2d.fromDegrees(0.0)),
            new Pose2d(Units.inchesToMeters(200.0), Units.inchesToMeters(70),
        ...
        DifferentialDrive drive = new DifferentialDrive(kRobotMassKg, kRobotAngularInertia,
            kRobotAngularDrag, kWheelRadius, Units.inchesToMeters(26.0 / 2.0), transmission,
            transmission);
        ...
        for (int i = 1; i < trajectory.size(); i++)
        {
            ...
            assertEquals(prev.state.acceleration, (next.state.velocity - prev.state.velocity) / dt,
                1E-9);
            assertEquals(next.state.velocity, prev.state.velocity + prev.state.acceleration * dt,
                1E-9);
        ...
        while (!it.isDone())
        {
            ...
            // This is designed to be exactly the same as 2019-DeepSpace output, as a
            // comparison
            ...
            // new Translation2d(
            // Units.metersToInches(state.state.getTranslation().x()),
            // Units.metersToInches(state.state.getTranslation().y())
            // ),
