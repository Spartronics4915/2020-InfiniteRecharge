...
public class PurePursuitTracker extends TrajectoryTracker
{
     ...
     *                                   end of the path.
     */
    public PurePursuitTracker(double lat, double lookaheadTimeSeconds,
        double minLookaheadDistanceMeters)
    {
        kLat = lat;
        ...
        TimedIterator<Pose2dWithCurvature> iterator, Pose2d currentRobotPoseMeters)
    {
        ...
        double xError = (referencePoint.state.state.getPose()
            .inFrameReferenceOf(currentRobotPoseMeters).getTranslation().getX());
        ...
        // Calculate the curvature of the arc that connects the robot and the lookahead
        // point
        double curvature = 2 * lookaheadTransform.getTranslation().getY() / Math.pow(l, 2);

        // Adjust the linear velocity proportional to the product of a gain and the
        // error, and the rotational error
        double adjustedLinearVelocity = vd * lookaheadTransform.getRotation().getCos()
            + kLat * xError;

        return new TrajectoryTrackerVelocityOutput(adjustedLinearVelocity,
            adjustedLinearVelocity * curvature // velocity * curvature = angular velocity
        );
    }

    private Pose2d calculateLookaheadPose(TimedIterator<Pose2dWithCurvature> iterator,
        Pose2d robotPose)
    {
        ...
        if (lookaheadPoseByTime.inFrameReferenceOf(robotPose).getTranslation()
            .norm() >= kMinLookaheadDistance)
        {
        ...
        while (iterator.getRemainingProgress() > previewedTime)
        {
            ...
            // E.g. in this situation iterator.preview(3) would preview at
            // iterator.getProgress() + 3
            lookaheadPoseByDistance = iterator.preview(previewedTime).state.state.getPose();
            double lookaheadDistance = lookaheadPoseByDistance.inFrameReferenceOf(robotPose)
                .getTranslation().norm();
        ...
        double remaining = kMinLookaheadDistance
            - (lookaheadPoseByDistance.inFrameReferenceOf(robotPose).getTranslation().norm());
        return lookaheadPoseByDistance.transformBy(new Pose2d(
            new Translation2d(remaining * (iterator.getTrajectory().isReversed() ? -1 : 1), 0.0),
            new Rotation2d()));
    }
