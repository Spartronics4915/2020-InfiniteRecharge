...
public abstract class TrajectoryTracker
{
    ...
    public TrajectoryTrackerOutput nextState(Pose2d currentRobotPoseMeters,
        double currentTimeSeconds)
    {
        ...
        if (iterator == null)
        {
            throw new RuntimeException(
                "You can't get the next state from the TrajectoryTracker without a trajectory! Call TrajectoryTracker::reset first.");
        }
        double deltaTime = mDeltaTimeController.updateTime(currentTimeSeconds);
        iterator.advance(deltaTime);

        TrajectoryTrackerVelocityOutput velocity = calculateState(iterator,
        ...
        if (previousVelocity == null || deltaTime <= 0)
        {
            ...
            return new TrajectoryTrackerOutput(velocity.linearVelocity, 0.0,
                velocity.angularVelocity, 0.0);
        }
        else
        {
            return new TrajectoryTrackerOutput(velocity.linearVelocity,
                (velocity.linearVelocity - previousVelocity.linearVelocity)
                    / deltaTime,
                velocity.angularVelocity,
                (velocity.angularVelocity - previousVelocity.angularVelocity)
                    / deltaTime);
        }
    }
    ...
    protected static class TrajectoryTrackerVelocityOutput
    {
        ...
        public TrajectoryTrackerVelocityOutput(double linearVelocityMetersSecond,
    ...
    public static class TrajectoryTrackerOutput
    {
        ...
        public TrajectoryTrackerOutput(double linearVelocityMetersSecond,
            double linearAccelerationMetersSecondSq, double angularVelocityRadsSecond,
        ...
        {
            return new DifferentialDrive.ChassisState(linearAcceleration,
                angularAcceleration);
        }
