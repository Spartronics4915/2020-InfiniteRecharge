...
public class TimedTrajectory<S extends State<S>> extends Trajectory<TimedTrajectory.TimedState<S>>
{
    ...
    public TrajectorySamplePoint<TimedState<S>> sample(double interpolant)
    {
        ...
        else
        {
            int index = IntStream.range(0, mPoints.size())
                .filter((i) -> i != 0 && mPoints.get(i).time >= interpolant).findFirst()
            ...
            {
                return new TrajectorySamplePoint<TimedState<S>>(lowerBoundState
                    .interpolate(upperBoundState, (interpolant - lowerBoundState.time)
                        / (upperBoundState.time - lowerBoundState.time)),
                    index - 1, index);
            }
        }
    }
    ...
    public static class TimedState<S extends State<S>> implements State<TimedState<S>>
    {
        ...
        public TimedState<S> interpolate(TimedState<S> endValue, double time)
        {
            ...
            boolean reversing = velocity < 0.0
                || Util.epsilonEquals(velocity, 0.0) && acceleration < 0.0;

            double newV = velocity + acceleration * deltaT;
            double newS = (reversing ? -1 : 1)
                * (velocity * deltaT + 0.5 * acceleration * deltaT * deltaT);

            return new TimedState<S>(
                state.interpolate(endValue.state, newS / state.distance(endValue.state)), newT,
                newV, acceleration);
        }
