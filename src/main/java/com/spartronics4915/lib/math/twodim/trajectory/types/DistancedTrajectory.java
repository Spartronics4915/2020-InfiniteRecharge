...
public class DistancedTrajectory<S extends State<S>> extends Trajectory<S>
{
    ...
    public DistancedTrajectory(List<S> points)
    {
        ...
        while (iter.hasNext())
        {
            ...
            mDistances
                .add(mDistances.get(mDistances.size() - 1) + firstPoint.distance(iter.next()));
    ...
    public TrajectorySamplePoint<S> sample(double interpolant)
    {
        ...
        else
        {
            int index = IntStream.range(0, mPoints.size())
                .filter((i) -> i != 0 && mDistances.get(i) >= interpolant).findFirst()
            ...
            else
            {
                return new TrajectorySamplePoint<S>(
                    lowerBoundState.interpolate(upperBoundState,
                        (interpolant - mDistances.get(index - 1))
                            / (mDistances.get(index) - mDistances.get(index - 1))),
                    index - 1, index);
            }
        }
    }
